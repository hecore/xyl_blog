...
<%@ include file="/WEB-INF/content/admins/side.jsp"%> 
  ...
  <div class="admin-content" id="admin-content">
	  ...
	  <div class="am-g">
	  	<div class="am-u-sm-12">
	  	   <!-- form组合提交后台 -->
			  ...
			  <div class="am-form-group">
			  	<label for="bozhu">用户</label>
			    <input id="bozhu" type="text" name="bozhu" value="${bozhu }" class="am-form-field" placeholder="用户">
			  </div>
			<div class="am-form-group">
		      <label for="video-level">等级</label>
		      <select id="video-level" name="video.blevel">
		        <c:if test="${empty video.blevel}">
	  ...
	  <div class="am-g">
		    ...
		    <tbody>
		       <c:forEach items="${videoPage.list }" var="l">
		       	<tr>
		       		...
		       		<td>${l.username }</td>
		       		<td>
		       			<c:if test="${l.blevel==0}">
		            		默认
		            	</c:if>
		            	<c:if test="${l.blevel==1 }">
		            		热门
		            	</c:if>
		            	<c:if test="${l.blevel==2 }">
		            		推荐
		            	</c:if>
		            	<c:if test="${l.blevel==3 }">
		            		置顶
		            	</c:if>
		       		</td>
		...
		<c:set var="actionUrl" value="${base}/admins/videoList?video.title=${video.title}&bozhu=${bozhu}&video.blevel=${video.blevel}&page=" />
...
<form id="videoleveleditform" action="${base}/admins/editVideoLevel/" method="post" style="padding: 20px;display: none;">
      ...
      <select id="videolevelselect" name="blevel">
        <option value="0">默认</option>
        <option value="1">热门</option>
        <option value="2">推荐</option>
        <option value="3">置顶</option>
