...
<%@ include file="/WEB-INF/content/common/taglib.jsp"%>
  ...
  <meta name="keywords" content="${blog.title},${blog.keyword},日志编辑">
  <meta name="viewport"
        content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">
  <title>编辑：${blog.title}</title>
  ...
  <!-- 引入kindeditor文件 -->
	...
	<script type="text/javascript">
		$(function(){
			 ...
			 $("input:radio[name='blog.zhuanzai'][value='${blog.zhuanzai}']").attr("checked","checked");
			 if(${blog.zhuanzai=="1"}){
	...
	<div class="am-container " id="main">
	  ...
	  <div class="am-g am-g-fixed">
	    <div class="am-u-md-8" id="blog-main">
			...
			<form class="am-form am-form-horizontal" id="blog-edit-form" method="post" action="${base}/blog/update" enctype="multipart/form-data">
				<input type="hidden" name="blog.id" value="${blog.id}">
				<input type="hidden" name="blog.user_id" value="${blog.user_id}">
				<input type="hidden" name="blog.deleted" value="${blog.deleted}">
				<input type="hidden" name="blog.bview" value="${blog.bview}">
			    ...
			    <div class="am-form-group am-form-group-sm">
			      ...
			      <div class="am-u-sm-10">
				      <input type="text"  name="blog.title" value="${blog.title}" id="blog-title" placeholder="输入日志标题"><span class="blog-error">${titleMsg}</span>
			      </div>
			    </div>
			    <div class="am-form-group am-form-group-sm">
			      <label class="am-u-sm-2 " for="blog-keyword">关键字</label>
			      <div class="am-u-sm-10">
				      <input type="text"  name="blog.keyword" value="${blog.keyword}" id="blog-keyword" placeholder="输入关键字，方便搜到你的日志">
			      </div>
			    </div>
			    <div class="am-form-group am-form-group-sm">
			    ...
			    <div class="am-form-group">
			      <label for="blog-content">日志内容</label>
			     <textarea id="blog-content" name="blog.content" cols="100" rows="8" style="width:100%;height:600px;visibility:hidden;"></textarea>
			    ...
			    <div class="am-form-group" >
			      ...
			      <span class="blog-error">${zhuanzaiurlMsg}</span>
			       <br/>
			       <c:if test="${blog.showside==0}">
				      <label class="am-radio-inline">
				        <input type="radio" value="1" name="blog.showside"  onclick="showside();">显示右侧区域
				      </label>
				      <label class="am-radio-inline">
				        <input type="radio" value="0" name="blog.showside" checked="checked" onclick="hideside();">隐藏右侧区域
				      </label>
			       </c:if>
			       <c:if test="${blog.showside==1}">
				      <label class="am-radio-inline">
			    ...
			    <input type="hidden" name="blog.editortype" value="0">
...
<script>
		...
		KindEditor.ready(function(K) {
			editor1 = K.create('textarea[name="blog.content"]', {
		...
		function dosubmit(){
			var editstr=editor1.text();
			var data = editstr.replace(/<img.*>.*<\/img>/ig,"");   //过滤如<img></img>形式的图片元素
			   data = data.replace(/<img.*\/>/ig, "").substr(0,150);
			$("#blog-content-show").val(data);
			$("#blog-content").val(editor1.html());
	...
	<script type="text/plain" id="hidecontent" style="display: none;" >${blog.content}</script>
</body>
</html>