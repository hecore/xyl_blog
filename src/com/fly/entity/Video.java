...
public class Video extends Model<Video>
{
    ...
    public Page<Video> paginate(int pageNumber, int pageSize)
    {
        return paginate(pageNumber, pageSize, "select * ",
                "from video order by blevel desc, create_time desc");
    }
    ...
    {
        return paginate(pageNumber, pageSize, "select * ",
                "from video where user_id=? order by blevel desc, create_time desc",
                userId);
    }
    ...
    {
        return find("select * from video where blevel=3");
    }
}
