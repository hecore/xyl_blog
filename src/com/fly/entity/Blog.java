...
public class Blog extends Model<Blog>
{
    ...
    {
        return Blogcategory.me.findById(Integer.parseInt(getNumber("category")+""));
    }
    ...
    public Page<Blog> paginate(int pageNumber, int pageSize)
    {
        return paginate(
                ...
                pageSize,
                "select * ",
                "from blog where ispublic=1 and blevel !=0 and deleted=0  order by blevel desc, create_time desc");
    }
    ...
    public Page<Blog> paginateHot(int pageNumber, int pageSize)
    {
        return paginate(
                ...
                pageSize,
                "select * ",
                "from blog b where b.ispublic=1 and b.deleted=0 order by b.bview desc, b.create_time desc");
        //这里防止view被Oracle识别不让使用
    }
    ...
    {
    	//  select * from blog where ispublic=1 and blevel=3 and deleted=0  order by  create_time desc  
        return find("select * from blog where ispublic=1 and blevel=3 and deleted=0  order by  create_time desc");
    }
}
