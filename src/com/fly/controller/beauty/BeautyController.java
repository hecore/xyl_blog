...
public class BeautyController extends BaseController
{
    public void index()
    {
        ...
        for (Beauty b : beautys)
        {
            ...
            every.put("id", Integer.parseInt(b.getNumber("id")+""));
            every.put("user_id", Integer.parseInt(b.getNumber("user_id")+""));
            every.put("title", b.getStr("title"));
            every.put("keyword", b.getStr("keyword"));
            every.put("bview", Integer.parseInt(b.getNumber("bview")+""));
    ...
    public void save()
    {
        ...
        beauty.set("user_id",
                Constants.getLoginUser(getSession()).getNumber("id"));
        beauty.set("title", beautytitle);
        beauty.set("img", g.toJson(imgRes));
        beauty.set("content", beautycontent);
        beauty.set("bview", 0);
        beauty.set("blevel", 0);
        beauty.set("deleted", 0);
        beauty.set("create_time", new java.sql.Timestamp(new java.util.Date().getTime()));//使用时间戳存入数据
    ...
    public void detail()
    {
        ...
        System.out.println(beauty.getNumber("bview"));
        beauty.set("bview", Integer.parseInt(beauty.getNumber("bview")+"")+ 1);
        beauty.update();
        beauty.set("content", HtmlUtil.Decode(beauty.getStr("content")));
        Users bozhu = Users.me.findById(Integer.parseInt(beauty.getNumber("user_id")+""));
    ...
    public void myBeauty()
    {
        ...
        beautylist = Beauty.me.myBeautyPaginate(getParaToInt("page", 1), 10,
                Integer.parseInt(user.getNumber("id")+""));
        setAttr("ranktype", "default");
        List<Beauty> beautys = beautylist.getList();
        List<Map> results = new ArrayList<Map>();
        for (Beauty b : beautys)
    ...
    public void delete()
    {
        ...
        Users user = Constants.getLoginUser(getSession());
        if (user.getNumber("id").intValue() != beauty.getNumber("user_id").intValue())
        {
            message = "非法操作";
            setAttr("message", message);
            renderJsp("/WEB-INF/content/common/result.jsp");
