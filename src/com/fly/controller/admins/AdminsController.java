...
public class AdminsController extends BaseController
{
    ...
    public void blogList()
    {
        ...
        {
            setAttr("bozhu", bozhu);
            List<Users> userlist = Users.me.findByUsername(bozhu);
            if ((!empty(userlist)) && userlist.size() >= 1)
            {
                Users u = userlist.get(0);
                sql += " and user_id=" + u.getNumber("id") + " ";
            }
        }
        if (!empty(searchBlog.getNumber("ispublic")))
        {
            sql += " and ispublic=" + searchBlog.getNumber("ispublic") + " ";
        }
        if (!empty(searchBlog.getNumber("level")))
        {
            sql += " and level=" + searchBlog.getNumber("level") + " ";
        }
        ...
        for (Blog b : blogs)
        {
            ...
            every.put("ispublic", b.getNumber("ispublic"));
            ...
            every.put("level", b.getNumber("level"));
    ...
    public void gonggaoSave()
    {
        ...
        if (empty(gonggao.getNumber("sn")))
        {
            gonggao.set("sn", 0);
            gonggao.set("type", 0);
            gonggao.set("deleted", 0);
            gonggao.set("create_time", new java.sql.Timestamp( new java.util.Date().getTime()));
            gonggao.save();
            gonggao.set("sn", gonggao.getNumber("id"));
            gonggao.update();
        }
        else
        {
    ...
    public void gonggaoUpdate()
    {
        getFile();
        Gonggao gonggao = getModel(Gonggao.class, "gonggao");
        if (empty(Integer.parseInt(gonggao.getNumber("id")+"")))
        {
            message = "参数错误";
            setAttr("message", message);
            renderJsp("/WEB-INF/content/common/result.jsp");
        ...
        Gonggao ug = Gonggao.me.findById(Integer.parseInt(gonggao.getNumber("id")+""));
        if (empty(gonggao.getNumber("sn")))
        {
            ug.set("content", gonggao.getStr("content"));
            ug.set("url", gonggao.getStr("url"));
            ug.set("sn", ug.getNumber("id"));
            ug.update();
        }
        else
        {
            ug.set("content", gonggao.getStr("content"));
            ug.set("url", gonggao.getStr("url"));
            ug.set("sn", gonggao.getNumber("sn"));
            ug.update();
        }
        message = "公告编辑成功";
        setAttr("message", message);
    ...
    public void adviceList()
    {
        ...
        for (Advice a : list)
        {
            ...
            {
                Users u = Users.me.findById(a.getInt("user_id"));
    ...
    public void adviceDetail()
    {
        ...
        {
            Users u = Users.me.findById(advice.getInt("user_id"));
    ...
    public void iplogList()
    {
        ...
        if (!empty(start) && !" ".equals(start))
        {
          ...
          //  sql = sql + " and create_time >='" + start + "'";
            sql=" and create_time >= TO_DATE('"+start+"','yyyy-mm-dd hh24:mi:ss')";
        }
        if (!empty(end) && !" ".equals(end))
        {
           ...
           // sql = sql + " and create_time <='" + end + "'";
            sql=" and create_time <= TO_DATE('"+end+"','yyyy-mm-dd hh24:mi:ss')";
        }
        if ((empty(start) || " ".equals(start))
                && (empty(end) || " ".equals(end)))
        {
           ...
           // sql = " and create_time>=" + start + " and create_time<=" + end;//mysql中的写法
            sql=" and create_time BETWEEN TO_DATE("+start+",'yyyy-mm-dd hh24:mi:ss')and TO_DATE("+end+",'yyyy-mm-dd hh24:mi:ss')";
        }
    ...
    public void resourcelogList()
    {
        ...
        String sql = "";
        // setAttr("end", end);
        //sql = sql + " create_time like'" + sdf.format(today) + "%'";//mysql写法
        sql = sql + " create_time " + "BETWEEN TO_DATE('"+sdf.format(today)+" 00:00:00 "+"','yyyy-mm-dd hh24:mi:ss')and TO_DATE('"+sdf.format(today)+" 23.59:59','yyyy-mm-dd hh24:mi:ss')";
        System.out.println(sql);
    ...
    public void resourcelogTongji()
    {
      ...
      //         + end + "'";
        String sql =" and create_time>=TO_DATE('" + start + "','yyyy-mm-dd hh24:mi:ss') and create_time<=TO_DATE('"+ end + "','yyyy-mm-dd hh24:mi:ss')";
        ...
        for (Resourceslog rl : list)
        {
            ...
            ynamePhymemory.add(Long.parseLong(rl.getNumber("phymemory")+"") / 1024.00);
            ynamePhyusedmemory
                    .add((Long.parseLong(rl.getNumber("phymemory")+"") - Long.parseLong(rl.getNumber("phyfreememory")+""))
                            / 1024.00);
            ynameDiskmemory.add(Long.parseLong(rl.getNumber("diskmemory")+"") / 1024.00);
            ynameDiskusedmemory.add(
                    (Long.parseLong(rl.getNumber("diskmemory")+"") - Long.parseLong(rl.getNumber("diskfreememory")+""))
                            / 1024.00);
            ynameJVMtotal.add(Long.parseLong(rl.getNumber("jvmtotalmemory")+"") / 1024.00);
            ynameJVMused.add(
                    (Long.parseLong(rl.getNumber("jvmtotalmemory")+"") - Long.parseLong(rl.getNumber("jvmfreememory")+""))
                            / 1024.00);
            ynameJVMsum.add(Long.parseLong(rl.getNumber("jvmmaxmemory")+"") / 1024.00);
            ynameGC.add(Long.parseLong(rl.getNumber("gccount")+""));
        }
        // CPU统计
    ...
    public void picRecommendSave()
    {
        ...
        pic.set("deleted", 0);
        pic.set("create_time", new java.sql.Timestamp( new java.util.Date().getTime()));
        if (Utility.empty(pic.getNumber("sn")))
        {
            pic.set("sn", 1);
            pic.save();
            pic.set("sn", pic.getNumber("id"));
            pic.update();
        }
        else
        {
    ...
    public void picRecommendUpdate()
    {
        ...
        Picrecommend pic = getModel(Picrecommend.class, "pic");
        Picrecommend old = Picrecommend.me.findById(Integer.parseInt(pic.getNumber("id")+""));
        if (Utility.empty(uf))
        {
        ...
        pic.set("deleted", old.getNumber("deleted"));
    ...
    public void beautyList()
    {
        ...
        if (!empty(searchBeauty.getNumber("blevel")))
        {
            sql += " and blevel=" + searchBeauty.getNumber("blevel") + " ";
        }
        ...
        for (Beauty b : blogs)
        {
            ...
            every.put("blevel", b.getNumber("blevel"));
    ...
    {
        String sql = "";
        Users searchUser = getModel(Users.class, "user");
        setAttr("user", searchUser);
        Page<Users> pagelist = null;
        ...
        pagelist = Users.me.adminPaginatListUsers(getParaToInt("page", 1), 10,
                sql);
        List<Users> users = pagelist.getList();
        List<Map> results = new ArrayList<Map>();
        for (Users u : users)
        {
            Map every = new HashMap();
            every.put("id", u.getNumber("id"));
    ...
    public void listDeletedUsers()
    {
        ...
        if (!empty(searchUser.getNumber("type")))
        {
            sql += " and type=" + searchUser.getNumber("type") + " ";
        }
        Page<Users> pagelist = null;
        pagelist = Users.me.adminListDeletedUses(getParaToInt("page", 1), 10,
                sql);
        List<Users> users = pagelist.getList();
        List<Map> results = new ArrayList<Map>();
        for (Users u : users)
    ...
    public void userRoleEdit()
    {
        ...
        System.out.println(u.getNumber("id")+"=========="+ u.getStr("username"));
        renderJson(m);
    }
    ...
    public void userRoleSave()
    {
        ...
        if (empty(ur) && roleId != 0)
        {
            ...
            userrole.set("create_time",new java.sql.Timestamp( new java.util.Date().getTime()));
    ...
    public void videoList()
    {
        ...
        if (!empty(searchVideo.getNumber("level")))
        {
            sql += " and blevel=" + searchVideo.getNumber("blevel") + " ";
        }
        ...
        for (Video v : videos)
        {
            ...
            every.put("level", v.getNumber("level"));
    ...
    public void linkSave()
    {
        ...
        link.set("create_time", new java.sql.Timestamp( new java.util.Date().getTime()));
        if (empty(link.getInt("sn")))
        {
            link.set("sn", 1);
            link.save();
            link.set("sn", link.getNumber("id"));
            link.update();
        }
        else
        {
